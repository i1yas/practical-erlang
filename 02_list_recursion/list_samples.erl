...
filter_users_sample() ->
    Users = [{user, 1, "Bob", male},
	     {user, 2, "Helen", female}, {user, 3, "Bill", male},
	     {user, 4, "Kate", female}],
    filter_female(Users, []).
...
split_by_age([], _, {Below, Under}) ->
    {lists:reverse(Below), lists:reverse(Under)};
split_by_age([User | Rest], Age, {Below, Under}) ->
    {_, UserAge} = User,
    if UserAge =< Age ->
