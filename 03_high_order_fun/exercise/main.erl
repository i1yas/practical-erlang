...
-export([filter_sick_players/1, get_stat/1,
	 make_pairs/2, sample_champ/0]).
...
get_stat(Champ) ->
    {NumTeams, NumPlayers, TotalAge, TotalRating} =
	lists:foldl(fun ({team, _, Players},
			 {NumTeams, NumPlayers, TotalAge, TotalRating}) ->
			    {TeamPlayersCount, TeamTotalAge, TeamTotalRating} =
				lists:foldl(fun ({player, _, Age, Rating, _},
						 {TeamPlayersCount,
						  TeamTotalAge,
						  TeamTotalRating}) ->
						    {TeamPlayersCount + 1,
						     TeamTotalAge + Age,
						     TeamTotalRating + Rating}
					    end,
					    {0, 0, 0}, Players),
			    {NumTeams + 1, NumPlayers + TeamPlayersCount,
			     TotalAge + TeamTotalAge,
			     TotalRating + TeamTotalRating}
		    end,
		    {0, 0, 0, 0}, Champ),
    {NumTeams, NumPlayers, TotalAge / NumPlayers,
     TotalRating / NumPlayers}.

get_stat_test() ->
    ?assertEqual({5, 40, 2.485e+1, 2.428e+2},
		 (get_stat(sample_champ()))),
    ok.

filter_sick_players(Champ) ->
    lists:filtermap(fun ({team, Name, Players}) ->
			    HealthyPlayers = lists:filter(fun ({player, _, _, _,
								Health}) ->
								  Health >= 50
							  end,
							  Players),
...
filter_sick_players_test() ->
    Result = [{team, "Crazy Bulls",
		...
		{player, "Beer Bear", 24, 241, 99}]}],
    ?assertEqual(Result,
		 (filter_sick_players(sample_champ()))),
    ok.

make_pairs({team, _, Players1}, {team, _, Players2}) ->
    [{Name1, Name2}
     || {player, Name1, _, Rating1, _} <- Players1,
...
make_pairs_test() ->
    ...
    ?assertEqual([{"Big Bull", "Lazy Horse"},
		  ...
		  {"Big Bull", "Horse Doors"}, {"Big Bull", "Rainbow"},
		  {"Big Bull", "HoHoHorse"}, {"Big Bull", "Pony"},
		  {"Big Bull", "Hippo"}, {"Big Bull", "Hop-Hop"},
    ...
    ?assertEqual([{"Ben The Hen", "Light Speed Cow"},
		  ...
		  {"Ben The Hen", "Cow Flow"}, {"Hen Hen", "Jumping Cow"},
		  {"Hen Hen", "Cow Flow"}, {"Son of Hen", "Boom! Cow"},
